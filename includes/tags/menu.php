						...
						<div class="nav-collapse row-fluid">
							...
							<ul class="nav clearfix navigation span12 <?php echo (is_home()) ? '' : 'external_page'; ?>" id="navigation">
								<?php
									$page_external_class='';
									$page_url='';
									$class_arr=array('red','orange','yellow','green','gray','brown','blue');	
									$f=0;
								?>
								...
								<?php if($pages_arr && !(is_string($pages_arr))) { ?>
									...
									?>
									<?php if($navItemObj->hide_nav_item === 'no' && stripos($navItemMenu['title'],'#')===false) { ?>
										<?php if($navItemMenu['brick_type'] === 'slogan') : ?>
											<li class="current<?php echo ' '.$page_external_class.' '.$class_arr[$f]; ?>"><a data-toggle="collapse" data-target=".nav-collapse" class="<?php echo 'page_'.$page_external_class; ?>" href="<?php echo $page_url; ?>"><img class="logo" src="<?php echo ot_get_option('site_logo'); ?>" alt="logo" /></a></li>
										<?php else : ?>
											<li class="<?php echo $page_external_class.' '.$class_arr[$f]; ?>"><a data-toggle="collapse" data-target=".nav-collapse" class="<?php echo 'page_'.$page_external_class; ?>" href="<?php echo $page_url; ?>"><?php echo $navItemMenu['title'];  $f++;?></a></li>
										<?php endif; ?>
									<?php } ?>
									<?php endforeach; ?>
								<?php } ?>
